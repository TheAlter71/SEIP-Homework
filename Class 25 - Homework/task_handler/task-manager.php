...
class TaskManager
{
    ...
    public function create($info)
    {
        ...
        if (isset($info['id'])) {
            ...
            $statement = $db->prepare("INSERT INTO car_infos(brand, color, price, photo) VALUES(:brand, :color, :price, :photo)");
            $statement->execute(
                [
                    'brand' => $info['brand'],
                    'color' => $info['color'],
    ...
    public function uploadImage($id, $name)
    {
        ...
        $statement = $db->prepare("UPDATE car_infos SET photo = :photo WHERE id = :id");
        $statement->execute(
            [
                'photo' => $name,
                'id' => $id
        ...
        // Toast value
        $_SESSION['toast'] = "Picture uploaded successfully!";
